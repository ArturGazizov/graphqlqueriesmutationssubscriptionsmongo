...
const [ editAuthor ] = useMutation(EDIT_AUTHOR, {
    ...
    update: (store, response) => {
      console.log('-->', response.data.editAuthor)
      
      const dataInStore = store.readQuery({ query: ALL_PERSONS })
      dataInStore.allAuthors.map((it)=>it.name==response.data.editAuthor.name?response.data.editAuthor:it)
      store.writeQuery({
        query: ALL_PERSONS,
        data: dataInStore
      })
...
editAuthor({ variables: { "born":parseInt(born),'name':name
    }}, {    refetchQueries: [ { query: ALL_BOOKS } ,{ query: ALL_PERSONS}]  })
